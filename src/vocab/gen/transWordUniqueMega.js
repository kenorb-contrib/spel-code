...
function returnIfUnique(transEntry, allDefinObj, index,
    ...
    allTransLangs.forEach(function (key) {
        ...
        if (matchingDefs.length <= 0 ||
            ...
            //console.log("Xword " + JSON.parse(thesaurus["Xword"]));
    ...
    if (foundBlanks > 0) {
        ...
        blacklist["X" + enDef] = thesaurusEntry;
                ...
                allTransLangs.length/Math.pow(1.618,1)) {
        result = "AMBIGIOUS"; 
...
function main() {
    var fileContents = io.fileRead("sortedComboList-mega.txt"),
        ...
        transEntry,
        uniqueJSON = io.fileRead("unique-mid.json"),
  ...
  specialWords = atomWords;
    ...
    io.fileWrite("comboUniqList-mega.txt", uniqText);
    io.fileWrite("thesaurus-mega.txt", formatThesaurus(thesaurus,
            mainWords));
    io.fileWrite("blacklist-mega.txt", formatThesaurus(blacklist,
            mainWords));
    io.fileWrite("suggestList-mega.txt", formatSuggestList(thesaurus, 
        blacklist, mainWords));
    ...
    io.fileWrite("unique-mega.json", JSON.stringify(outObj));
}
